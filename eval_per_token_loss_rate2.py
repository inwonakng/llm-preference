...
@click.option('--bin_range', default=120, type=int, help='Range to bin the word count by.')
...
def evaluate(
    ...
    by_bin = pd.DataFrame()
    by_bin['row_count'] = short_zero_shot['row_count']
    by_bin['Max Wordcount'] = short_zero_shot['Max Wordcount']
    prompt = [short_zero_shot, short_few_shot, long_zero_shot, long_few_shot]
    prompt_name = ['short_zero_shot', 'short_few_shot', 'long_zero_shot', 'long_few_shot']
    col_name = ['3 Class Correct', '4 Class Correct']
    for i in col_name:
    ...
    for i in by_bin.columns:
        if i == 'row_count' or i == 'Max Wordcount': continue
    ...
    error_rate_4 = []
    for i in by_bin.columns:
        if i.endswith('Rate') and i.startswith('3'):
            error_rate_3.append(i)
        if i.endswith('Rate') and i.startswith('4'):
            error_rate_4.append(i)

    by_bin.plot(x = "Max Wordcount", y = error_rate_3, kind = "line", color = ["blue", "red", "green", "yellow"], ax = ax1) 
    ax1.set_ylabel('3 Class Error Rate')

    by_bin.plot(x = "Max Wordcount", y = error_rate_4, kind = "line", color = ["blue", "red", "green", "yellow"], ax = ax2) 
    ...
    # fig.savefig(fig_save_dir / 'eval_per_token_loss_rate2.pdf', bbox_inches='tight')
    fig.savefig('eval_per_token_sikai2_lose_rate.pdf', bbox_inches='tight')
    print('save success')
